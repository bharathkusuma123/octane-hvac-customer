...
setErrorCount(alarmValue && alarmValue !== "0" ? Number(alarmValue) : 0);
 ...
 const handlePowerToggle = async () => {
  ...
  const payload = {
    ...
    MD: sensorData.mode, // Use current mode from state
    FS: sensorData.fanSpeed, // Use current fan speed from state
    SRT: sensorData.temperature, // Use current temperature from state
    HVAC: newHvacValue,
    Footer: "0xZX",
  };
